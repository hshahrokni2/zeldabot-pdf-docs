...
def create_brf_landing_page(building_data, market_position, total_buildings):
    ...
    with col1:
        ...
        st.plotly_chart(fig_financial, use_container_width=True, key=f"brf_financial_gauge_{building_data['building_id']}")
    ...
    with col1:
        ...
        st.plotly_chart(fig_pie, use_container_width=True, key=f"brf_cost_breakdown_pie_{building_data['building_id']}")
    ...
    with tab1:
        ...
        if selected_brf_name:
            ...
            with col1:
                ...
                # Handle building marker clicks to navigate to BRF profile
                if map_data["last_object_clicked_tooltip"]:
                    clicked_tooltip = map_data["last_object_clicked_tooltip"]
                    # Extract BRF name from tooltip (format: "BRF Name - Energy Class X")
                    if " - Energy Class " in clicked_tooltip:
                        brf_name = clicked_tooltip.split(" - Energy Class ")[0]
                        # Find the clicked building in the data
                        clicked_building = next((b for b in filtered_buildings if b['brf_name'] == brf_name), None)
                        if clicked_building:
                            st.session_state.selected_brf_profile = clicked_building['brf_name']
                            st.success(f"🏢 Loading profile for **{clicked_building['brf_name']}**...")
                            st.rerun()
            ...
            if show_survey or st.session_state.get('show_survey', False):
                ...
                if 'current_survey' not in st.session_state or not st.session_state.current_survey.get('building_id'):
                    ...
                    if sjostaden_2:
                        ...
                        # Pre-populate with comprehensive cost categories to unlock savings analysis
                        st.session_state.current_survey['costs'] = {
                            ...
                            'recycling': 145159.0,       # From building data
                            'snow_removal': 157709.0,    # From building data
                            'administration': 120000.0,  # Estimated administrative costs
                            'security': 80000.0,         # Estimated security costs
                            'gardening': 45000.0         # Estimated gardening costs
                        }
                        st.session_state.current_survey['suppliers'] = {
                            ...
                            'recycling': 'SYSAV',
                            'snow_removal': 'Vinterservice Stockholm',
                            'administration': 'Interreg Förvaltning',
                            'security': 'Securitas',
                            'gardening': 'GreenTeam Stockholm'
                        }
